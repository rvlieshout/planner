...
| `workflow_states` | `team_id`, `name`, `type`, `position`, `is_default` | Board columns. Teams rename and reorder freely; `type` (Backlog/Unstarted/Started/Completed/Canceled) is the stable meaning behind the name, so "is this done?" survives a rename. |
| `labels` | `team_id` **nullable**, `name`, `color` | `NULL` team means organisation-wide. |
| `projects` | `team_id`, `name`, `status`, `health`, `lead_user_id`, `start_date`, `target_date` | Progress is **not** stored; it is counted from issues at read time so it cannot drift. |
| `milestones` | `project_id`, `name`, `target_date`, `status`, `sort_order` | Ordered checkpoints. Deleting one leaves its issues in the project. |
...
| `issues` | `team_id`, `number`, `state_id`, `priority`, `assignee_id`, `project_id`, `milestone_id`, `parent_id`, `sort_order` | The unit of work. `(team_id, number)` is unique. Lifecycle stamps (`started_at`, `completed_at`, `canceled_at`) are driven by the target state's `type`. |
...
| `ix_issues_team_id_state_id_sort_order` | The board query: one team, one column, in rank order. |
...
| `ix_milestones_project_id_sort_order` | Ordered milestone lists. |
