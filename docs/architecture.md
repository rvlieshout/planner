...
Two grants are enabled: `password` and `refresh_token`. Authorization-code + PKCE is the better
pattern in general, but it needs an interactive login page and a browser; a first-party desktop client
on a company network has neither, and adding a login UI to a headless API is a lot of surface for no
security gain when the API is the only thing holding the passwords anyway. The upgrade path is real
though — see [roles-and-permissions.md](roles-and-permissions.md#upgrading-to-authorization-code--pkce).
...
- **Attachments are metadata only.** The API stores a filename and a `storage_uri`; bytes live on a
  share or an S3-compatible store. No upload endpoint ships here.
